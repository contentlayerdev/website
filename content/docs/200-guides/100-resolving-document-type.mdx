...
Any document can explicitly declare its type. Do this by adding a field to the document called `type` and using the `name` option from the document type definition. [See here](/docs/reference/local-source/define-document-type) for all document type options.
