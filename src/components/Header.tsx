...
export const Header = () => {
  ...
  return (
    <header className="fixed z-10 flex justify-between w-full px-6 items-center bg-white border-b bg-opacity-90 backdrop-filter backdrop-blur-sm h-[60px]">
      <Link href="/">
        ...
        {(headerConfig.nav ?? []).map((link, idx) => {
          ...
          return (
            <Link key={idx} href={link.url}>
              <a className={classNames('text-gray-900', { 'font-bold': isActive })}>{link.label}</a>
            </Link>
          )
        })}
        <GitHubLogoLink />
